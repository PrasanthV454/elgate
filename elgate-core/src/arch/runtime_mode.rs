...
impl fmt::Debug for RuntimeMode {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::PinnedSharded { worker_count, numa_aware } => {
                write!(f, "PinnedSharded(workers={}, numa={})", worker_count, numa_aware)
            }
...
mod tests {
    ...
    fn test_select_runtime_mode() {
        ...
        let mode = RuntimeMode::select_for_system(&cpu_info);
        assert!(matches!(mode, RuntimeMode::PinnedSharded { worker_count: 2, .. }));
        
        // Test 8-core system
        let cpu_info = CpuInfo::mock(8, 0);
        let mode = RuntimeMode::select_for_system(&cpu_info);
        assert!(matches!(mode, RuntimeMode::PinnedSharded { worker_count: 7, .. }));
        
        // Test 8-core system with 2 NUMA nodes
        let cpu_info = CpuInfo::mock(8, 2);
        let mode = RuntimeMode::select_for_system(&cpu_info);
        if let RuntimeMode::PinnedSharded { worker_count, numa_aware } = mode {
